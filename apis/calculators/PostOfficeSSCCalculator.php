...
class PostOfficeSSCCalculator extends BaseCalculator{
	var $principal;
	var $rate ;
	var $time;
	var $amount;
	...
	public function output($params){
		...
		$this->quarterly_interest = ceil($this->total_interest / (4 * $this->time)); 
		$data = $this->getCalculator($params);
		$output = $data['output'];
		$output = $this->render(array(
				'amount' => $this->INRFormat($this->total_interest) , 
				'quarterly_interest' => $this->INRFormat($this->quarterly_interest), 
				'total_interest' => $this->INRFormat($this->total_interest), 
	...
	public function addPieChart($params){
		...
		$datasets = array();
		$data = array('labels' => array('Total Interest Earned' ,  'Deposit Amount'));
