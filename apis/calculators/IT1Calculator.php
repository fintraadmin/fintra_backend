...
class IT1Calculator extends BaseCalculator{
	var $principal;
	var $rate ;
	var $time;
	var $amount;
	...
	public function output($params){
		...
		$output = $this->render(array(	
						...
						'tax1'=> $this->INRFormat($this->tax1),
						'cess1'=> $this->INRFormat($this->cess1),
						'surcharge1'=> $this->INRFormat($this->surcharge1),
						'totaltax1'=> $this->INRFormat($this->totaltax1),
	...
	public function calculation2020(){
		$slab0 = 250000;
		$slab1 = 500000;
                $slab2 = 750000;
                $relief = 500000;
		...
		$t5 = 0.3;
                ...
                if($this->income <= $slab1){
                        ...
                        $tax = $t1 * ($this->income - $slab1) + $t0* ($slab1 - $slab0);
                }
                else if($this->income <= $slab3){
                        $tax = $t1 * ($slab2 - $slab1) + $t0* ($slab1 - $slab0) + $t2 * ($this->income - $slab2);
                }
                else if($this->income <= $slab4){
                        $tax = $t2 * ($slab3 - $slab2) + $t1 * ($slab2 -$slab1)+ $t0* ($slab1 - $slab0) + $t3 * ($this->income - $slab3);
                }
                else if($this->income <= $slab5){
                        $tax =  $t3* ($slab4 - $slab3) +  $t2 * ($slab3 - $slab2) + $t1 * ($slab2 -$slab1) + $t0* ($slab1 - $slab0) + $t4 * ($this->income - $slab4);
                }
		else if($this->income > $slab5){
                        $tax =  $t4 * ($slab5 - $slab4) + $t3 * ($slab4 - $slab3) +  $t2 * ($slab3 - $slab2) + $t1 * ($slab2 -$slab1) + $t0* ($slab1 - $slab0) + $t5 * ($this->income - $slab5);
		}
                //add surcharge;
                ...
                $this->tax1 = $tax;
                $this->cess1 = $cess;
                $this->surcharge1 = $surcharge;
                $this->totaltax1 = $surcharge + $cess + $tax;
                $this->taxpercent1 = round($this->totaltax1 / $this->income *100 ,2);
