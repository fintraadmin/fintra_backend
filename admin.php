...
$template = $twig->load('a_list_application.twig.html');
...
try {
    ...
    $result = $appDao->listApplications($cid);
    $applications = $result['applications'];
    foreach($applications as &$a){
		$aid=$a['merchant_id'];
		$a['url']= "tracking?cid=$cid&aid=$aid&lead_id=". $a['lead_id'];
		if($a['income_source'] == 2){
			$a['loan_type'] = 'Business';
		}
		if($a['income_source'] == 1){
			$a['loan_type'] = 'Personal';
		}
		if($a['doc_uploaded'] == 0){
